...
assert.match(sw, /CACHE\s*=\s*['"]orbit-neural-core-v2-panorama-r8['"]/, 'panorama release must invalidate the previous service-worker cache');
for(const ref of ['styles.css?v=6','start-v2.css?v=21','neural-core-v2.js?v=7','start-v2.js?v=21'])assert.ok(html.includes(`"${ref}"`), `canonical entry must use fresh cinematic asset ${ref}`);
...
assert.doesNotMatch(start, /audio\.loop=true/, 'cinematic score must never loop');
