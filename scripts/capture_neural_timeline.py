...
async def main():
    ...
    async with websockets.connect(target["webSocketDebuggerUrl"], max_size=20_000_000) as ws:
        ...
        await call("Runtime.evaluate", {"expression": "ORBITNeuralCore.pushVoiceFrame({active:true,rms:.8,low:.65,mid:.5,high:.3})"})
        await asyncio.sleep(.1)
        envelope_attack = await call("Runtime.evaluate", {"expression": "JSON.stringify(ORBITNeuralCore.telemetry())", "returnByValue": True})
        await call("Runtime.evaluate", {"expression": "ORBITNeuralCore.pushVoiceFrame({active:false,rms:0,low:0,mid:0,high:0})"})
        await asyncio.sleep(.08)
        envelope_hold = await call("Runtime.evaluate", {"expression": "JSON.stringify(ORBITNeuralCore.telemetry())", "returnByValue": True})
        await asyncio.sleep(.5)
        envelope_release = await call("Runtime.evaluate", {"expression": "JSON.stringify(ORBITNeuralCore.telemetry())", "returnByValue": True})
        print("voice-envelope", envelope_attack["result"]["value"], envelope_hold["result"]["value"], envelope_release["result"]["value"])
        await call("Runtime.evaluate", {"expression": "window.dispatchEvent(new PointerEvent('pointermove',{clientX:380,clientY:300,pointerType:'touch'}))"})
        await asyncio.sleep(1.6)
        parallax_right = await call("Runtime.evaluate", {"expression": "JSON.stringify(ORBITNeuralCore.telemetry())", "returnByValue": True})
        await call("Runtime.evaluate", {"expression": "window.dispatchEvent(new PointerEvent('pointermove',{clientX:10,clientY:300,pointerType:'touch'}))"})
        await asyncio.sleep(1.6)
        parallax_left = await call("Runtime.evaluate", {"expression": "JSON.stringify(ORBITNeuralCore.telemetry())", "returnByValue": True})
        print("parallax", parallax_right["result"]["value"], parallax_left["result"]["value"])
